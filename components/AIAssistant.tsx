  ...
  useEffect(() => {
    const checkApiKey = async () => {
        ...
        if (win.aistudio) {
            ...
            // If the helper isn't available, assume we are in an env where key is manually set or handle error gracefully later
            setHasApiKey(true);
        }
    };
    checkApiKey();
                    ...
                    {!hasApiKey ? (
                        <div className="flex flex-col items-center justify-center space-y-3 p-4">
                            <p className="text-gray-600 dark:text-slate-300 text-sm text-center">
                                To use the Health Assistant, please connect your Google Gemini API Key.
                                <br />
                                <a href="https://ai.google.dev/gemini-api/docs/billing" target="_blank" rel="noopener noreferrer" className="text-brand-blue underline hover:text-blue-700">
                                    Learn about API Keys and billing
                                </a>
                            </p>
                            <button
                                onClick={handleConnect}
                                className="px-6 py-3 bg-brand-teal text-white rounded-full font-semibold shadow-md hover:bg-teal-700 transition-colors"
                            >
                                Connect API Key
                            </button>
                        </div>
